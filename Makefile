...
OBJECTS += $(patsubst Sources/%.cpp,build/$(ARCHDIR)BTTT/%.o,$(patsubst ../RaptorEngine/%.cpp,build/$(ARCHDIR)RaptorEngine/%.o,$(SOURCES)))
OBJDIRS += $(dir $(OBJECTS))
...
%.app: universal
	...
	cp -p "BTTT.icns" "$@/Contents/Resources/"
...
build/$(ARCHDIR)BTTT/%.o: Sources/%.cpp $(GAME_HEADERS) $(ENGINE_HEADERS)
	@mkdir -p $(dir $@)
...
clean:
	rm -rf build/arch-* build/BTTT.res "$(PRODUCT)" "$(EXE)" "$(EXE)"_*

install:
	mkdir -p "$(GAMEDIR)"
	-rsync -ax --exclude=".*" *.dll "$(GAMEDIR)/"
	-rsync -ax --exclude=".*" Bin* "$(GAMEDIR)/"
	-rsync -ax --exclude=".*" Fonts "$(GAMEDIR)/"
